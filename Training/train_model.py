			...
			for i in range(NUM_GPUS):
				...
				with tf.device('/gpu:%d' % i):
					...
					with tf.name_scope('%s_%d' % ('tower', i)) as scope:
						...
						logits, penultimate, _ = model.inference(x)
...
#						FLAGS.max_epochs = 1  # TEST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
						...
						for i in range(FLAGS.max_epochs):
							...
							avg_l2_loss_i = 0
							print("Training steps: %s" %training_steps)
							# ------------ TRAIN -------------
							...
							if i % FLAGS.eval_freq == 0 or i == FLAGS.max_epochs-1:
								...
								if tot_loss_entropy < min_loss or i == 0: #> FLAGS.max_epochs * 0.6:
									max_acc = tot_acc
...
#						   fmt='%.5ef')
				...
				print('Acc/std/AUC/std : %s/%s' % (avg_acc, std_acc)) #, avg_auc, std_auc))

...
# === MSO keeping output size intact ===
def _rfnn_conv_layer_pure_2d_MSO_max(input, basis, omaps, is_training, bn_mom=None, rn_mom=None, strides=[1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
...
def _rfnn_conv_layer_pure_2d_MSO_avg(input, basis, omaps, is_training, bn_mom=None, rn_mom=None, strides=[1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
			shape=[basis.get_shape()[2].value, input.get_shape()[-1].value, omaps],
...
def _rfnn_conv_layer_pure_2d_MSO_learn_sq_bc(input, basis, omaps, is_training, bn_mom=None, rn_mom=None, strides=[1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
			shape=[basis.get_shape()[2].value, input.get_shape()[-1].value, omaps],
	...
	return output, alphas, kernels, scale_bc

def _rfnn_conv_layer_pure_2d_MSO_learn_fl_bc(input, basis, omaps,  is_training, bn_mom=None, rn_mom=None, strides=[1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
			shape=[basis.get_shape()[2].value, input.get_shape()[-1].value, omaps],
...
def _rfnn_conv_layer_pure_2d_MSO_learn_flatten(input, basis, omaps, is_training, bn_mom, rn_mom, strides=[1, 1, 1, 1], padding='SAME'):
...
def _rfnn_conv_layer_pure_3d_MSO_learn_sq_bc(input, basis, omaps, strides=[1, 1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
			shape=[basis.get_shape()[2].value, input.get_shape()[-1].value, omaps],
...
def _rfnn_conv_layer_pure_3d_MSO_learn_fl_bc(input, basis, omaps, strides=[1, 1, 1, 1, 1], padding='SAME'):
	with tf.device('/cpu:0'):
		alphas = tf.get_variable(
			'alphas',
			shape=[basis.get_shape()[2].value, input.get_shape()[-1].value, omaps],
	...
	for i, sigma in enumerate(sigmas):
		...
		for t in range(orients1):
			for f in range(orients2):
				...
				hermiteBasis[i, t * orients2 + f, 4, :, :, :] = \
					+ np.power(alpha, 4) * conv(conv(gauss0y, g, axis=2), g4, axis=1) \
					+ np.power(beta, 4) * conv(conv(gauss0x, g, axis=2), g4, axis=0) \
					+ np.power(gamma, 4) * conv(conv(gauss0x, g, axis=0), g4, axis=2) \
					+ 4 * np.power(alpha, 2) * beta * conv(conv(gauss1y, g, axis=2), g3, axis=1) \
					+ 4 * np.power(alpha, 2) * gamma * conv(conv(gauss1z, g, axis=0), g3, axis=1) \
					+ 4 * np.power(beta, 2) * alpha * conv(conv(gauss1x, g, axis=2), g3, axis=0) \
					+ 4 * np.power(beta, 2) * gamma * conv(conv(gauss1z, g, axis=1), g3, axis=0) \
					+ 4 * np.power(gamma, 2) * alpha * conv(conv(gauss1x, g, axis=0), g3, axis=2) \
					+ 4 * np.power(gamma, 2) * beta * conv(conv(gauss1y, g, axis=1), g3, axis=2) \
					+ 6 * np.power(alpha, 2) * np.power(beta, 2) * conv(conv(gauss0z, g2, axis=1), g2, axis=0) \
					+ 6 * np.power(alpha, 2) * np.power(gamma, 2) * conv(conv(gauss0y, g2, axis=1), g2, axis=2) \
					+ 6 * np.power(gamma, 2) * np.power(beta, 2) * conv(conv(gauss0x, g2, axis=0), g2, axis=2) \
					+ 12 * np.power(alpha, 2) * beta * gamma * conv(conv(gauss1y, g1, axis=2), g2, axis=1) \
					+ 12 * alpha * np.power(beta, 2) * gamma * conv(conv(gauss1x, g1, axis=2), g2, axis=0) \
					+ 12 * alpha * beta * np.power(gamma, 2) * conv(conv(gauss1x, g1, axis=0), g2, axis=2) \

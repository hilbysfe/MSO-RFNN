...
from Utils.cnn_utils import _conv_layer_decomp3d
...
#			output = self.batch_norm(_input)
			...
			#				self.bc_conv_act.append(output)
				...
				if self.decomp:
					output, weights_spat, weights_temp = _conv_layer_decomp3d(output, shape=[kernel_size, kernel_size, kernel_size,
																 int(output.get_shape()[-1]), out_features])
					self.variable_dict[scope.name+"_spatial"] = weights_spat
					self.variable_dict[scope.name+"_temporal"] = weights_temp
				else:
...
#			output = self.batch_norm(_input)
	...
	def inference(self, X):
		...
		with tf.variable_scope("Initial_convolution") as scope:
			if self.decomp:
				output, weights_spat, weights_temp = _conv_layer_decomp3d(
					X,
					shape=[self.initial_kernel, self.initial_kernel, self.initial_kernel, int(X.get_shape()[-1]),
						   self.first_output_features],	strides=[1, 2, 2, 2, 1])
				self.variable_dict[scope.name+"_spatial"] = weights_spat
				self.variable_dict[scope.name+"_temporal"] = weights_temp
			else:
			...
			print(output.get_shape())
#			self.kernels.append(weights_spat)
#			self.kernels.append(weights_temp)
			self.conv_act.append(output)
